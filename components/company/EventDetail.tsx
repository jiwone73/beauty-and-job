...
// list-full.png 는 왼쪽에 지역·직군 필터 사이드바가 있어, 메인 자리와 같은
// 왼쪽 값을 쓰면 상자가 사이드바까지 먹는다 — PlanDetail.tsx 의 목록왼·목록오
// 와 같은 값을 쓴다(카드가 실제로 시작하는 자리를 잰 값).
const 목록자리 = { 위: 9.2, 높이: 14.8, 왼: 18.4, 오: 2.2 };
      ...
      <section className="ev-sec">
        ...
        <ul className="pi-warn">
          ...
          <li>
            ...
            무료 공고 {스타트.무료건수}건과 달리 건수 제한 없이 올리실 수 있습니다.
          </li>
          <li>오픈이벤트 채용관은 정해진 칸을 차례로 교대하며, 먼저 올리신 순서대로 앞자리에 노출됩니다.</li>
          <li>
            1개월이 끝나면 자동 결제 없이 {스타트.name}(무료 회원)으로 돌아가고 올려 두신 공고는
            노출이 끝납니다. 계속 노출하시려면 {플랜.LIGHT.name}부터 신청하시면 됩니다.
          </li>
