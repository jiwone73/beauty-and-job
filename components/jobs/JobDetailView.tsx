...
export function ImageCarousel({ images, alt }: { images: string[]; alt?: string }) {
  ...
  // 배너는 '고정 높이'(가로세로비 고정)를 유지한다 — 공고마다 배너 높이가 달라지면 안 됨.
  //   여러 장을 같은 폭으로 나눠 넣고, 각 이미지는 위·아래를 균등하게 잘라(center crop) 칸을 꽉 채운다.
  ...
  return (
    <div style={{ position: "relative", width: "100%" }}>
      <div style={{ display: "flex", gap: 0, aspectRatio: "15 / 7", borderRadius: 12, overflow: "hidden", background: "#f4f4f4" }}>
        {visible.map((src, k) => (
