...
export default function Step10Done() {
  ...
  const { name, job, jobCustom, careerYears, isLeader, categories, categoryCustom, countries, countryCustom } = useSignupStore();
  ...
  return (
    <div className="p-7 px-8 pt-12 text-center">
      ...
      <h2 className="text-2xl font-bold text-[#1a1a1a] tracking-tight mb-3">환영합니다, <span className="text-primary">{name || "회원"}</span>님!</h2>
      <p className="text-[15px] text-[#6b6b6b] leading-relaxed mb-6">뷰티앤잡 가입이 완료되었습니다<br />입력하신 정보로 맞춤 채용공고를 추천해드릴게요</p>
      <div className="bg-[#f7f7f8] rounded-lg p-4 mb-7 text-left">
        {summary.map((row, idx) => (
          <div key={row.label} className={`flex justify-between gap-3 py-2 text-[13px] ${idx < summary.length - 1 ? "border-b border-[#ececec]" : ""}`}>
            <span className="text-[#9a9a9a] flex-shrink-0">{row.label}</span>
            <span className="text-[#1a1a1a] font-medium text-right break-keep leading-relaxed">{row.value}</span>
          </div>
        ))}
      </div>
      <button type="button" onClick={handleGoProfile} className="btn-primary-full active">내 프로필 보러가기</button>
      <button type="button" onClick={handleGoMain} className="block mx-auto mt-4 px-3 py-1.5 text-sm text-[#6b6b6b] hover:text-primary transition-colors">메인으로 이동</button>
    </div>
  );
}
