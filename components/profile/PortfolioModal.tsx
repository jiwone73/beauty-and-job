...
const 본사칩: 채움[] = [
  ...
  { 이름: "브런치",     앞부분: "brunch.co.kr/@", 별칭: ["brunch", "브런치"] },
];

// 아무것도 안 쳤을 때 무엇을 먼저 보여줄지만 갈린다. 글자를 치기 시작하면
// 양쪽 모두에서 찾는다 — 매장 회원이 노션을 쓸 수도 있는데 못 찾게 막을
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: 8 }}>
              {골라줄것.map((k) => (
                <button key={k.이름} type="button"
                  ...
                  style={{ ...글, padding: "5px 10px", borderRadius: "var(--chip-radius)", border: "1px solid #efeff1", background: "#f7f7f8", color: "#582681", cursor: "pointer" }}>
                  {k.이름}
                </button>
              ))}
            </div>
