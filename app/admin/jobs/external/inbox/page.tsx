...
export default function ExternalInboxPage() {
  ...
  const forward = async (r: Row) => {
    ...
    try {
      ...
      const j = await res.json();
      if (!j.success) { alert(j.error?.message || "전달 실패"); }
      await load();
  ...
  return (
    <AdminLayout activeMenu="jobs-external-inbox">
      <div style={{ maxWidth: 1200 }}>
        <h1 style={{ fontSize: 22, fontWeight: 800, marginBottom: 4 }}>외부 지원 인박스</h1>
        <p style={{ fontSize: 13.5, color: "#888", marginBottom: 18 }}>
          미가입 기업 공고에 들어온 지원을 확인하고, 해당 기업에 전달·상태 관리합니다.
        </p>
        ...
        <div style={{ display: "flex", gap: 12, marginBottom: 16 }}>
          {[["전달 대기", counts.PENDING, true], ["전달됨", counts.FORWARDED, false], ["실패", counts.FAILED, false], ["가입 전환", counts.claimed, false]].map(([l, v, warn], i) => (
            <div key={i} style={{ flex: 1, background: warn ? "#fff7f1" : "#fff", border: `1px solid ${warn ? "#ffd9be" : "#eee"}`, borderRadius: 12, padding: "14px 18px" }}>
              <div style={{ fontSize: 24, fontWeight: 800, color: warn ? "#FA6400" : "#1a1a1a" }}>{v as number}</div>
              <div style={{ fontSize: 12.5, color: "#888", marginTop: 2 }}>{l as string}</div>
            </div>
          ))}
        </div>
            ...
            <table className="admin-table" style={{ width: "100%" }}>
              <thead>
                <tr>
                  ...
                  <th>지원일</th>
                  <th>상태</th>
                  <th className="rt" style={{ textAlign: "right" }}>처리</th>
                </tr>
              </thead>
              <tbody>
                {shown.map((r) => {
