...
const JOB_GROUPS = {
  "기업·사무직": {
    "브랜드·마케팅": ["브랜드 마케터", "디지털·퍼포먼스 마케터", "콘텐츠 마케터", "SNS·인플루언서 마케팅", "글로벌 마케팅", "CRM 마케터"],
    ...
    "경영지원": ["HR·채용", "재무·회계", "경영기획·전략", "CS·CX", "법무·컴플라이언스"],
  },
  "매장·기술직": {
    "네일": ["네일 아티스트", "젤네일 전문가", "네일샵 매니저", "네일 강사"],
    "헤어": ["헤어 디자이너", "헤어 어시스턴트", "헤어 컬러리스트", "살롱 매니저"],
    "피부·에스테틱": ["피부관리사", "에스테티션", "피부 관리실 원장", "피부 강사"],
    "메이크업": ["메이크업 아티스트", "뷰티 크리에이터", "메이크업 강사"],
    "기타 뷰티": ["속눈썹·눈썹 아티스트", "왁싱 전문가", "반영구 아티스트", "뷰티샵 스태프"],
  },
};

const BEAUTY_CATEGORIES = ["스킨케어", "색조", "헤어·바디", "향수", "더마코스메틱", "클린뷰티", "선케어", "맨즈케어", "건기식", "디바이스", "OEM·ODM", "플랫폼·유통", "MCN·미디어", "카테고리 무관"];
const REGIONS = ["서울", "경기·인천", "부산·경남", "대구·경북", "광주·전남", "대전·충청", "기타 지방", "일본", "북미", "동남아", "중국", "유럽", "글로벌"];
const CAREER_OPTIONS = ["신입", "1년 이상", "2년 이상", "3년 이상", "5년 이상", "7년 이상", "10년 이상", "경력 무관"];
const EMPLOYMENT_TYPES = ["정규직", "계약직", "인턴", "아르바이트", "프리랜서"];

export default function AdminJobNewPage() {
  const router = useRouter();
  const [jobGroupType, setJobGroupType] = useState<"기업·사무직" | "매장·기술직">("기업·사무직");
  const [selectedGroup, setSelectedGroup] = useState("");
  const [form, setForm] = useState({
    company: "", title: "", category: "", jobGroup: "", career: "",
    region: "", type: "정규직", deadline: "", salary: "",
    beautyCategory: "", description: "", requirements: "", preferred: "", benefits: "",
  });
  ...
  const handleSubmit = (status: "draft" | "publish") => {
    if (!form.company.trim()) { alert("기업명을 입력해주세요."); return; }
    ...
    <AdminLayout activeMenu="jobs">
      ...
      <div className="admin-form-grid">
          ...
          <div className="admin-form-body">
            ...
            <div className="admin-form-row">
              ...
              <div className="admin-job-type-tabs">
                {(["기업·사무직", "매장·기술직"] as const).map((t) => (
                  <button key={t} type="button"
                    className={`admin-job-type-tab ${jobGroupType === t ? "active" : ""}`}
                    onClick={() => {
                      setJobGroupType(t);
                      setSelectedGroup("");
                      setForm({ ...form, category: "", jobGroup: "" });
                    }}>
                    {t === "기업·사무직" ? "🏢 기업·사무직" : "💄 매장·기술직"}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="admin-form-row">
              <label className="admin-form-label">
                {jobGroupType === "매장·기술직" ? "매장명 *" : "기업명 *"}
              </label>
              <input className="admin-form-input"
                placeholder={jobGroupType === "매장·기술직" ? "예) 네일살롱 강남점" : "기업명을 입력하세요"}
                value={form.company}
            ...
            <div className="admin-form-row">
              ...
              <input className="admin-form-input"
                placeholder={jobGroupType === "매장·기술직" ? "예) 네일 아티스트 모집" : "예) 마케팅 매니저 (색조)"}
                value={form.title}
            ...
            <div className="admin-form-row">
              ...
              <select className="admin-form-select" value={selectedGroup}
                onChange={(e) => {
                  ...
                  setForm({ ...form, category: "", jobGroup: e.target.value });
                }}>
            ...
            {jobGroupType === "기업·사무직" && (
              <div className="admin-form-row">
                <label className="admin-form-label">담당 카테고리</label>
                <select className="admin-form-select" value={form.beautyCategory}
                  onChange={(e) => setForm({ ...form, beautyCategory: e.target.value })}>
                  <option value="">선택</option>
                  {BEAUTY_CATEGORIES.map(o => <option key={o} value={o}>{o}</option>)}
                </select>
              </div>
            )}
            ...
            <div className="admin-form-row-2col">
              <div className="admin-form-row">
                <label className="admin-form-label">
                  {jobGroupType === "매장·기술직" ? "급여" : "연봉"}
                </label>
                <input className="admin-form-input"
                  placeholder={jobGroupType === "매장·기술직" ? "예) 월 250만원 / 협의" : "예) 4,000~5,000만원"}
                  value={form.salary}
                  onChange={(e) => setForm({ ...form, salary: e.target.value })} />
              </div>
              <div className="admin-form-row">
                <label className="admin-form-label">마감일</label>
                <input className="admin-form-input" placeholder="예) 2025.02.28 또는 채용시 마감"
                  value={form.deadline}
        ...
        <div className="admin-card">
          ...
          <div className="admin-form-body">
            <div className="admin-form-row">
              <label className="admin-form-label">포지션 소개 *</label>
              <textarea className="admin-form-textarea"
                placeholder={jobGroupType === "매장·기술직"
                  ? "매장 소개 및 모집 포지션에 대해 입력하세요"
                  : "이 포지션에 대한 소개를 입력하세요"}
                value={form.description}
                onChange={(e) => setForm({ ...form, description: e.target.value })} />
            </div>
            <div className="admin-form-row">
              <label className="admin-form-label">자격요건 *</label>
              <textarea className="admin-form-textarea"
                placeholder={jobGroupType === "매장·기술직"
                  ? "예) 네일 자격증 보유자\n경력 1년 이상\n성실하고 고객응대 가능하신 분"
                  : "필수 자격요건을 입력하세요"}
            ...
            <div className="admin-form-row">
              <label className="admin-form-label">우대사항</label>
              <textarea className="admin-form-textarea" placeholder="우대사항을 입력하세요"
                value={form.preferred}
                onChange={(e) => setForm({ ...form, preferred: e.target.value })} />
            </div>
            <div className="admin-form-row">
              <label className="admin-form-label">
                {jobGroupType === "매장·기술직" ? "근무조건·복지" : "복리후생"}
              </label>
              <textarea className="admin-form-textarea"
                placeholder={jobGroupType === "매장·기술직"
                  ? "예) 주 5일 근무\n4대보험\n재료비 지원\n교육비 지원"
