...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      const fileName = `${companyId}/${Date.now()}.${줄인.ext}`;
...
export async function DELETE(req: NextRequest) {
  ...
  try {
    ...
    const oldUrl = res.rows[0]?.logo_url;
    if (oldUrl) {
      const oldPath = oldUrl.split(`/${BUCKET}/`)[1];
      if (oldPath) {
        await supabaseAdmin.storage.from(BUCKET).remove([oldPath]);
    ...
    await client.query(
      `UPDATE companies SET logo_url = NULL WHERE id = $1`,
      [companyId]
    );
    return ok({ deleted: true });
  } catch (e) {
    console.error("[company logo delete]", e);
