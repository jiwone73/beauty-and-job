 ...
 *
 * 스탠다드에도 한때 같이 줬는데, 그러면 프리미엄이 더 파는 것이 「알림」
 * 한 줄뿐이라 값을 더 낼 이유가 약했다. 스탠다드는 「내가 찾는다」,
 * 프리미엄은 「찾지 않아도 온다」로 나눈다 — 목록과 알림을 한 등급에 묶는다.
 *
...
export async function GET(req: NextRequest) {
  ...
  const 열림 = plan === "PREMIUM";
  if (!열림) {
    // 못 쓴다는 사실만 알린다. 화면이 「프리미엄부터」라고 적을 수 있게.
