  ...
  try {
    ...
    const counts = await client.query(`
      SELECT
        ...
        (SELECT COUNT(*) FROM users WHERE (created_at AT TIME ZONE 'Asia/Seoul')::date = (now() AT TIME ZONE 'Asia/Seoul')::date) AS today_users,
        (SELECT COUNT(*) FROM users WHERE (created_at AT TIME ZONE 'Asia/Seoul')::date = (now() AT TIME ZONE 'Asia/Seoul')::date AND job_type = 'OFFICE') AS today_users_office,
        (SELECT COUNT(*) FROM users WHERE (created_at AT TIME ZONE 'Asia/Seoul')::date = (now() AT TIME ZONE 'Asia/Seoul')::date AND job_type = 'STORE') AS today_users_store,
        ...
        (SELECT COUNT(*) FROM applications WHERE (applied_at AT TIME ZONE 'Asia/Seoul')::date = (now() AT TIME ZONE 'Asia/Seoul')::date) AS today_applications,
        (SELECT COUNT(*) FROM job_postings WHERE (created_at AT TIME ZONE 'Asia/Seoul')::date = (now() AT TIME ZONE 'Asia/Seoul')::date) AS today_jobs,
        (SELECT COUNT(*) FROM job_postings WHERE (created_at AT TIME ZONE 'Asia/Seoul')::date = (now() AT TIME ZONE 'Asia/Seoul')::date AND job_type = 'STORE') AS today_jobs_store,
        (SELECT COUNT(*) FROM job_postings WHERE (created_at AT TIME ZONE 'Asia/Seoul')::date = (now() AT TIME ZONE 'Asia/Seoul')::date AND job_type = 'OFFICE') AS today_jobs_office,
    ...
    let today_logins: number | null = null
    try {
      const v = await client.query(`
        SELECT COUNT(*)::int AS n,
               COUNT(*) FILTER (WHERE user_id IS NOT NULL)::int AS logins
          FROM site_visits WHERE visit_date = (now() AT TIME ZONE 'Asia/Seoul')::date`)
      today_visitors = v.rows[0]?.n ?? 0
      today_logins = v.rows[0]?.logins ?? 0
    } catch {}

    return ok({
      counts: { ...counts.rows[0], today_visitors, today_logins },
