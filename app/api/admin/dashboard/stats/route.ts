...
export async function GET(req: NextRequest) {
  ...
  const trendCfg =
    ...
    : range === '1m'
      ...
      : { start: "(now() AT TIME ZONE 'Asia/Seoul')::date - interval '6 day'", step: "interval '1 day'", trunc: 'day' }
  ...
  try {
    ...
    const counts = await client.query(`
      SELECT
        ...
        (SELECT COUNT(*) FROM users WHERE created_at::date = (now() AT TIME ZONE 'Asia/Seoul')::date) AS today_users,
        (SELECT COUNT(*) FROM users WHERE created_at::date = (now() AT TIME ZONE 'Asia/Seoul')::date AND job_type = 'OFFICE') AS today_users_office,
        (SELECT COUNT(*) FROM users WHERE created_at::date = (now() AT TIME ZONE 'Asia/Seoul')::date AND job_type = 'STORE') AS today_users_store,
        ...
        (SELECT COUNT(*) FROM applications a JOIN job_postings jp ON jp.id = a.job_posting_id WHERE a.applied_at::date = (now() AT TIME ZONE 'Asia/Seoul')::date AND jp.job_type = 'OFFICE') AS today_applications_office,
        (SELECT COUNT(*) FROM applications a JOIN job_postings jp ON jp.id = a.job_posting_id WHERE a.applied_at::date = (now() AT TIME ZONE 'Asia/Seoul')::date AND jp.job_type = 'STORE') AS today_applications_store,
        ...
        (SELECT COUNT(*) FROM companies WHERE created_at::date = (now() AT TIME ZONE 'Asia/Seoul')::date) AS today_companies,
        (SELECT COUNT(*) FROM companies WHERE created_at::date = (now() AT TIME ZONE 'Asia/Seoul')::date AND company_type = 'STORE') AS today_companies_store,
        (SELECT COUNT(*) FROM companies WHERE created_at::date = (now() AT TIME ZONE 'Asia/Seoul')::date AND company_type = 'OFFICE') AS today_companies_office,
        (SELECT COUNT(*) FROM companies WHERE created_at::date = (now() AT TIME ZONE 'Asia/Seoul')::date AND company_type = 'BOTH') AS today_companies_both,
        ...
        (SELECT COUNT(*) FROM applications WHERE applied_at::date = (now() AT TIME ZONE 'Asia/Seoul')::date) AS today_applications,
        (SELECT COUNT(*) FROM job_postings WHERE created_at::date = (now() AT TIME ZONE 'Asia/Seoul')::date) AS today_jobs,
        (SELECT COUNT(*) FROM job_postings WHERE created_at::date = (now() AT TIME ZONE 'Asia/Seoul')::date AND job_type = 'STORE') AS today_jobs_store,
        (SELECT COUNT(*) FROM job_postings WHERE created_at::date = (now() AT TIME ZONE 'Asia/Seoul')::date AND job_type = 'OFFICE') AS today_jobs_office,
