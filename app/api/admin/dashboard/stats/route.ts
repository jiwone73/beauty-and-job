...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const counts = await client.query(`
      SELECT
        (SELECT COUNT(*) FROM users WHERE is_sample = false) AS total_users,
        (SELECT COUNT(*) FROM users WHERE job_type = 'OFFICE' AND is_sample = false) AS office_users,
        (SELECT COUNT(*) FROM users WHERE job_type = 'STORE' AND is_sample = false) AS store_users,
        (SELECT COUNT(*) FROM users WHERE created_at::date = now()::date AND is_sample = false) AS today_users,
        (SELECT COUNT(*) FROM users WHERE created_at::date = now()::date AND job_type = 'OFFICE' AND is_sample = false) AS today_users_office,
        (SELECT COUNT(*) FROM users WHERE created_at::date = now()::date AND job_type = 'STORE' AND is_sample = false) AS today_users_store,
        ...
        (SELECT COUNT(*) FROM companies WHERE is_sample = false) AS total_companies,
        (SELECT COUNT(*) FROM companies WHERE company_type = 'STORE' AND is_sample = false) AS store_companies,
        (SELECT COUNT(*) FROM companies WHERE company_type = 'OFFICE' AND is_sample = false) AS office_companies,
        (SELECT COUNT(*) FROM companies WHERE company_type = 'BOTH' AND is_sample = false) AS both_companies,
        (SELECT COUNT(*) FROM companies WHERE created_at::date = now()::date AND is_sample = false) AS today_companies,
        (SELECT COUNT(*) FROM companies WHERE created_at::date = now()::date AND company_type = 'STORE' AND is_sample = false) AS today_companies_store,
        (SELECT COUNT(*) FROM companies WHERE created_at::date = now()::date AND company_type = 'OFFICE' AND is_sample = false) AS today_companies_office,
        (SELECT COUNT(*) FROM companies WHERE created_at::date = now()::date AND company_type = 'BOTH' AND is_sample = false) AS today_companies_both,
        (SELECT COUNT(*) FROM companies WHERE status = 'PENDING' AND is_sample = false) AS pending_companies,
        (SELECT COUNT(*) FROM job_postings WHERE status = 'ACTIVE' AND is_sample = false AND (deadline IS NULL OR deadline >= CURRENT_DATE)) AS active_jobs,
        (SELECT COUNT(*) FROM job_postings WHERE status = 'ACTIVE' AND job_type = 'STORE' AND is_sample = false AND (deadline IS NULL OR deadline >= CURRENT_DATE)) AS active_jobs_store,
        (SELECT COUNT(*) FROM job_postings WHERE status = 'ACTIVE' AND job_type = 'OFFICE' AND is_sample = false AND (deadline IS NULL OR deadline >= CURRENT_DATE)) AS active_jobs_office,
        (SELECT COUNT(*) FROM job_postings jp JOIN companies c2 ON c2.id = jp.company_id WHERE jp.status = 'ACTIVE' AND c2.company_type = 'BOTH' AND jp.is_sample = false AND (jp.deadline IS NULL OR jp.deadline >= CURRENT_DATE)) AS active_jobs_both,
        (SELECT COUNT(*) FROM job_postings WHERE job_type = 'OFFICE' AND status = 'ACTIVE' AND is_sample = false AND (deadline IS NULL OR deadline >= CURRENT_DATE)) AS office_jobs,
        (SELECT COUNT(*) FROM job_postings WHERE job_type = 'STORE' AND status = 'ACTIVE' AND is_sample = false AND (deadline IS NULL OR deadline >= CURRENT_DATE)) AS store_jobs,
        (SELECT COUNT(*) FROM applications WHERE applied_at::date = now()::date) AS today_applications,
        (SELECT COUNT(*) FROM job_postings WHERE created_at::date = now()::date AND is_sample = false) AS today_jobs,
        (SELECT COUNT(*) FROM job_postings WHERE created_at::date = now()::date AND job_type = 'STORE' AND is_sample = false) AS today_jobs_store,
        (SELECT COUNT(*) FROM job_postings WHERE created_at::date = now()::date AND job_type = 'OFFICE' AND is_sample = false) AS today_jobs_office,
