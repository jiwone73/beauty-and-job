...
// 자동 측정이 놓친 시간을 관리자가 넣어 주는 창구.
//
// 본인은 못 쓴다. 자기 근무 시간을 스스로 적을 수 있으면 기록이 근거가 되지 못한다.
// 알바가 빠진 시간을 주장하면 관리자가 확인하고 넣는다.
...
export async function POST(req: NextRequest) {
  ...
  if (isTracked(auth!.sub)) return err("AUTH_002", "본인 근무 기록은 직접 넣을 수 없어요.", 403);
  ...
  const endDate =
      ...
      ? new Date(Date.parse(`${date}T00:00:00+09:00`) + 86400000).toISOString().slice(0, 10)
      : date;
...
export async function DELETE(req: NextRequest) {
  const { auth, res: authErr } = requireAuth(req, "admin");
  if (authErr) return authErr;
  if (isTracked(auth!.sub)) return err("AUTH_002", "본인 근무 기록은 직접 지울 수 없어요.", 403);
