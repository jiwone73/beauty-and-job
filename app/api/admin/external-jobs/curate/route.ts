...
export async function POST(req: NextRequest) {
  ...
  const hasAny = [src.title, src.company_description, src.description, src.responsibilities, src.requirements, src.preferred, src.benefits].some((v) => v.trim());
...
{"title","company_description","description","responsibilities","requirements","preferred","benefits"}
규칙:
...
- 입력에 없는 정보는 절대 추가하지 말 것.`;
    ...
    if (parsed && typeof parsed === "object") {
      ...
      for (const k of ["title", "company_description", "description", "responsibilities", "requirements", "preferred", "benefits"]) {
        let v = parsed[k];
