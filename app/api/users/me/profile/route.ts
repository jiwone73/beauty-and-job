...
export async function GET(req: NextRequest) {
  ...
  const [profile, careers, educations, experiences, languages, links, certificates, me] = await Promise.all([
    ...
    pool.query(`SELECT job_type, avatar_public, email, portfolio_images, (password_hash IS NOT NULL) AS has_password FROM users WHERE id = $1`, [userId]),
  ]);
  ...
  return ok({
    ...
    // 완성도를 세려면 포트폴리오 사진 수가 필요하다. 없으면 화면이 /api/users/me 를
    // 한 번 더 부르게 된다.
    portfolio_images: me.rows[0]?.portfolio_images ?? [],
    profile: profileData,
