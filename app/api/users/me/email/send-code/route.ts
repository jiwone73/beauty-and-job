...
// 개인 이메일 변경 1단계: (비밀번호 계정이면) 비밀번호 확인 + 새 이메일로 인증코드 발송
export async function POST(req: NextRequest) {
  ...
  if (hash) {
    // 비밀번호 로그인 계정은 비밀번호 확인 (소셜 계정은 비밀번호가 없어 생략)
  ...
  await pool.query(
    `UPDATE refresh_tokens SET revoked_at = NOW() WHERE revoked_at IS NULL AND token_hash LIKE $1`,
    [`email_change_user:${auth!.sub}:%`]
  );
  await pool.query(
    ...
    [`email_change_user:${auth!.sub}:${newEmail}:${code}`, expiresAt]
  );
  ...
  try {
    ...
    if (result?.error) {
      ...
      console.error("[email-change-user] resend error", result.error);
    }
  } catch (e: any) {
    sent = false;
    sendError = e?.message || String(e);
    console.error("[email-change-user] send fail", e);
  }

  const devCode = process.env.NODE_ENV !== "production" ? code : undefined;
  return ok({ sent, needs_password: !!hash, ...(sendError ? { error: sendError } : {}), ...(devCode ? { dev_code: devCode } : {}) });
}
