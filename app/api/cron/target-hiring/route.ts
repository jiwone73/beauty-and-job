...
import { closeGoneJobs, sweepSourceUrls } from "@/lib/external/closeGoneJobs";
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const r = await checkHiringFor(targets, { maxPages: 1, gapMs: 1000, deadlineMs: 35_000 });
    // 목록에서 사라진 원문 중, 우리가 올린 공고가 있는 것만 직접 열어 보고 마감시킨다.
    // 열어 보는 값이라 확실할 때만 내려간다(못 열면 그냥 둔다).
    const gone = await closeGoneJobs(r.gone, { max: 5 });

    // 외부업체 리스트에 없는 공고(카페·인스타에서 옮겨 온 개인 매장)도 돌아가며 살핀다.
    // 남은 시간 안에서만 — 무료 요금제 함수가 60초에서 끊긴다.
    const sweep = await sweepSourceUrls({ max: 8, deadlineAt: startedAt + 52_000 });
    ...
    return ok({
      ...
      sourceChecked: sweep.checked,
      sourceClosed: sweep.closed,
