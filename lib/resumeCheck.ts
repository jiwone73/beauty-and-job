...
export function 이력서흠찾기(입력: {
  ...
  // ── 학력 ── 학력 구분·학교명은 매장도 필수다. 졸업 상태·전공은 본사에서만 묻는다(고졸에게는 적을 것이 없다).
  ...
  educations.forEach((e) => {
    ...
    // 매장은 학력 구분과 학교명만 필수다. 졸업 상태·기간·전공은 본사에서만 묻는다.
    if (본사냐 && 빔(e.status)) 흠들.push({ 어디: "education", 누구: e.id, 말: "졸업 상태를 골라 주세요." });
