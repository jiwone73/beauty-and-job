...
export function 이력서흠찾기(입력: {
  ...
  if (!isEntryLevel) {
    ...
    careers.forEach((c) => {
      ...
      // 매장은 직급을 골라 넣는다 — 무슨 자리에서 일했는지가 경력의 무게다.
      if (!본사냐 && 빔(c.position)) 흠들.push({ 어디: "career", 누구: c.id, 말: "직급을 골라 주세요." });
      // 주요 성과는 필수가 아니다(별표도 뺐다) — 안 적었다고 저장하기를 막지 않는다.
    });
  }
  ...
  // ── 스킬·어학 ── 스킬은 매장·오피스 모두 1개 이상 필수. 어학은 선택이다.
  if (skills.length === 0) 흠들.push({ 어디: "skill", 말: "스킬을 1개 이상 넣어 주세요." });
