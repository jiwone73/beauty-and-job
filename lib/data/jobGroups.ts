...
export const SEARCH_TAGS: Record<string, string[]> = {
  ...
  "헤어 스텝": ["assistant", "헤어스탭", "스탭", "스태프", "스텝", "인턴", "어시", "샴푸", "와인딩", "리셉션", "막내", "수습", "스페어", "스페아", "단기", "주말", "승급", "매장리셉션", "주니어", "시니어"],
...
// 시술 직군의 사다리. 실장은 헤어에만 있는 자리가 아니다 — 네일샵·피부샵·
// 두피샵에도 실장이 있고, 그 자리를 뽑는 공고도 그렇게 적는다. 예전에는
// 헤어만 실장을 갖고 있어서 다른 시술 직군의 실장이 「경력」에 묻혔다.
export const 시술단계 = ["인턴", "신입", "경력", "실장"] as const;
...
// 본사도 인턴을 뽑는다 — 브랜드 마케팅 인턴, MD 인턴은 흔한 자리다. 없으면
// 그 사람들이 「신입」에 묻히고, 매장에는 인턴이 있는데 본사만 빠져 어긋난다.
export const 사무단계 = ["인턴", "신입", "1~2년", "3~5년", "5~10년", "10년+"] as const;
...
const 단계표: Record<string, readonly string[]> = {
  ...
  "미용강사": ["신입", "경력"],
  // 소분류가 샵매니저·뷰티매니저인데 매니저급을 못 고르면 그 자리를 찾는
  // 사람과 뽑는 매장이 서로 안 보인다.
  "샵 운영·상담": ["신입", "경력", "매니저급", "점장급"],
};
