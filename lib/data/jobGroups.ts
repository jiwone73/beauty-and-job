...
//
// 구조: 매장(STORE) / 오피스(OFFICE) → 대분류(1st Depth) → 소분류(2nd Depth)
//  - group  = 대분류 (모달 왼쪽 패널)
...
// 매장과 오피스는 대분류 이름이 겹치지 않는다. 겹쳐 두었더니(「뷰티 리테일」과
// 「뷰티 리테일 & 커머스」, 양쪽의 「의료미용」) 고르는 사람이 점장 자리와
// BM 자리를 같은 것으로 읽었다.
...
// ───────────── 오피스직 (OFFICE) : 기업·오피스직 ─────────────
// 대분류는 **무슨 일을 하는가**로 나눈다. 매장 쪽이 「헤어·바버 / 메이크업」처럼
// 직무로 서 있는데 오피스만 「제조·OEM·ODM / 플랫폼·콘텐츠」처럼 업종으로 서
// 있었다. 자기를 「제조·OEM·ODM 하는 사람」이라 부르는 구직자는 없다 —
// 연구원이거나 MD거나 마케터다.
//
// 업종 축이 실제로 깨뜨린 것:
//  · 경영지원(인사·재무·기획)이 「제조·OEM·ODM」 안에 있었다. 오피스 공고에서
//    제일 많은 직무인데 제조 칩을 눌러야 나왔고, 플랫폼 회사 인사담당 공고는
//    들어갈 자리가 없었다.
//  · 영업이 세 대분류에 흩어져 해외 영업 하는 사람이 어느 칩을 눌러야 할지
//    알 수 없었다.
//
// 오피스는 경력 사다리가 전부 연차(사무단계)라 대분류가 사다리를 가르지 않는다.
// 대분류가 하는 일은 「이 사람에게 어떤 공고를 먼저 보여줄까」 하나뿐이고,
// 그 판단은 업종이 아니라 직무로 해야 맞는다.
export const OFFICE_JOB_GROUPS: JobGroup[] = [
  ...
  {
    // 지금은 VMD 하나지만 패키지·그래픽 디자이너가 들어올 자리다. 한 칸짜리
    // 대분류를 두는 까닭은 축을 직무로 못 박아 두기 위해서다.
    group: "디자인",
    items: [
      "VMD·매장 디스플레이 디자이너",
    ],
  ...
  {
    // 매장의 「미용강사」와 짝이다. 그쪽은 학원 현장에서 가르치고, 여기는
    // 브랜드가 제품·시술을 가르친다.
    group: "교육",
    items: [
      "브랜드 에듀케이터",
...
export const SEARCH_TAGS: Record<string, string[]> = {
  ...
  // 네일 & 속눈썹
  // 현장에서 제일 많이 쓰는 이름은 「네일리스트」다. 우리 목록 이름(네일 아티스트)과
  // 글자가 달라, 공고 제목이 「네일리스트 모집」이면 직군을 못 찾았다.
  "네일 아티스트": ["nail", "네일", "젤", "네일아트", "파츠", "technician", "젤네일", "네일아트샵", "네일리스트", "네일아티스트", "네일디자이너"],
...
/**
 * 화면에 적는 이름. 이 두 낱말은 여기서만 정한다.
 *
 * 여태 서른 몇 개 파일이 저마다 "본사"라고 적어 두어, 말 하나 바꾸는 데
 * 220줄을 훑어야 했다. 다음에 또 바꿀 일이 생기면 여기만 고친다.
 *
 * 「본사」를 버린 까닭: 본사는 지점이 있어야 성립하는 말이라 다섯 명짜리
 * 브랜드·OEM 공장·플랫폼에는 없는 것을 있다고 우긴다. 영어 corporate 에는
 ...
 * @param query   입력 문자열
 * @param jobType 특정 트랙만 검색하려면 지정 (없으면 매장·오피스 전체)
...
export const 관리단계 = ["인턴", "신입", "매니저급", "점장급"] as const;
// 오피스는 자리가 아니라 연차로 뽑는다. 뷰티 오피스 채용을 다루는 코공고도
// 전부 「경력 3-5년」·「신입 ~ 경력 5년」으로 적지 직급으로 적지 않는다.
// 「리드」는 IT 말이라 우리 공고 325건에서 한 번도 안 골렸다.
// 오피스도 인턴을 뽑는다 — 브랜드 마케팅 인턴, MD 인턴은 흔한 자리다. 없으면
// 그 사람들이 「신입」에 묻히고, 매장에는 인턴이 있는데 오피스만 빠져 어긋난다.
...
const 단계표: Record<string, readonly string[]> = {
  ...
  // 사람과 뽑는 매장이 서로 안 보인다.
  "샵 운영·상담": ["인턴", "신입", "경력", "매니저급", "점장급"],
};

/** 대분류의 경력 단계. 오피스는 직군을 가리지 않고 한 사다리를 쓴다.
 *  이름이 겹치지 않게 정리했지만 jobType 은 그대로 받는다 — 매장 대분류와
 *  오피스 대분류를 한 표에서 찾으면 나중에 같은 이름이 생기는 날 다시 어긋난다. */
