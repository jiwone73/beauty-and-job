...
// 매장과 본사는 대분류 이름이 겹치지 않는다. 겹쳐 두었더니(「뷰티 리테일」과
...
// 직무로 서 있는데 본사만 「제조·OEM·ODM / 플랫폼·콘텐츠」처럼 업종으로 서
// 있었다. 자기를 「제조·OEM·ODM 하는 사람」이라 부르는 구직자는 없다 —
// 연구원이거나 MD거나 마케터다.
//
// 업종 축이 실제로 깨뜨린 것:
//  · 경영지원(인사·재무·기획)이 「제조·OEM·ODM」 안에 있었다. 본사 공고에서
//    제일 많은 직무인데 제조 칩을 눌러야 나왔고, 플랫폼 회사 인사담당 공고는
//    들어갈 자리가 없었다.
//  · 영업이 세 대분류에 흩어져 해외 영업 하는 사람이 어느 칩을 눌러야 할지
//    알 수 없었다.
//
// 본사는 경력 사다리가 전부 연차(사무단계)라 대분류가 사다리를 가르지 않는다.
 ...
 *  본사 대분류를 한 표에서 찾으면 나중에 같은 이름이 생기는 날 다시 어긋난다. */
