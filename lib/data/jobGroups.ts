...
export const STORE_JOB_GROUPS: JobGroup[] = [
  ...
  {
    // 살롱이 신입 교육을 맡을 강사를 뽑고, 아카데미 자체가 매장 형태인 경우도
    // 많다. 본사에도 같은 이름의 대분류가 선다 — 「뷰티 리테일 & 커머스」·
    // 「의료미용」처럼, 같은 일도 현장과 본사에서 하는 몫이 다르다.
    group: "교육·아카데미",
    items: [
      "미용 강사(헤어·메이크업·네일·피부)",
      "아카데미 원장·교육 실장",
  ...
  {
    // 업종에는 「애견미용」이 이미 있는데 직군이 없어, 애견샵이 공고를 내면
    // 모집분야에 고를 것이 없었다. 헤어에 섞으면 헤어 디자이너를 찾는 검색에
    // 딸려 나온다 — 제 대분류를 준다.
    group: "반려동물 미용",
    items: [
      "애견미용사(펫 그루머)",
...
export const SEARCH_TAGS: Record<string, string[]> = {
  // 교육·아카데미
  "미용 강사(헤어·메이크업·네일·피부)": ["강사", "미용강사", "선생님", "교육", "아카데미", "학원", "instructor", "teacher", "실기강사", "국가고시"],
  "아카데미 원장·교육 실장": ["원장", "교육실장", "아카데미", "학원", "교육기획", "academy"],
  "신입 교육 트레이너": ["트레이너", "교육", "신입교육", "OJT", "인턴교육", "trainer"],
  "브랜드 에듀케이터": ["에듀케이터", "educator", "브랜드교육", "제품교육", "세미나", "트레이너"],
  "교육 콘텐츠 기획": ["교육기획", "커리큘럼", "콘텐츠", "교재", "이러닝"],
  // 반려동물 미용
  "애견미용사(펫 그루머)": ["애견", "애견미용", "펫", "펫미용", "그루머", "groomer", "반려동물", "강아지", "미용사", "펫샵", "동물병원"],
  "애견미용 스탭·인턴": ["애견", "펫", "그루머", "스탭", "인턴", "보조", "목욕"],
  "애견 스파·목욕 관리사": ["애견", "펫", "스파", "목욕", "위생미용", "그루밍"],
  // 헤어 & 바버
  ...
  "에스테틱 상담 실장": ["상담", "실장", "세일즈", "consultant", "카운셀러"],
  ...
  "두피·탈모 상담 실장·뷰티 카운셀러": ["상담", "실장", "카운셀러", "세일즈", "회원권"],
