...
export const STORE_JOB_GROUPS: JobGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      "문제성 네일 교정 전문가",
      "속눈썹·반영구 아티스트",
      // 왁싱은 분류표에 없었는데, 파서는 이미 왁싱 공고를 속눈썹·반영구로
      // 보내고 있었다. 화면에 그 말이 없어 아무도 그렇게 읽을 수 없었다.
      "왁싱·제모 전문가",
      "네일 스탭·인턴",
    ],
  },
...
export const SEARCH_TAGS: Record<string, string[]> = {
  ...
  "헤어 스탭(시니어·주니어)": ["assistant", "헤어스탭", "스탭", "스태프", "스텝", "인턴", "어시", "샴푸", "와인딩", "리셉션", "막내", "수습", "스페어", "스페아", "단기", "주말"],
  // 공고는 '스페아'로도 적는다(실제 외부 공고 3건 중 2건이 그 표기였다).

  // 메이크업
  "메이크업 아티스트": ["makeup", "make-up", "mua", "메이크업", "아티스트", "프리랜서", "출장", "freelance"],
  ...
  "왁싱·제모 전문가": ["waxing", "왁싱", "제모", "브라질리언", "슈가링", "eyebrow wax"],
  "속눈썹·반영구 아티스트": ["eyelash", "속눈썹", "연장", "속눈썹펌", "반영구", "semi-permanent"],
  "네일 스탭·인턴": ["스탭", "스태프", "스텝", "인턴", "staff", "네일"],

  // 스킨 & 바디케어
  "피부 관리사(일반·경락)": ["aesthetician", "피부관리사", "에스테틱", "에스테티션", "경락", "페이셜", "관리사"],
  ...
  "모발이식·탈모 메디컬 스탭": ["hair transplant", "모발이식", "메디컬", "탈모", "생착"],
  ...
  // 뷰티 리테일(매장)
  "매장 점장·샵마스터(직영)": ["store manager", "점장", "샵마스터", "매니저", "직영"],
  ...
  // HR 서비스·채용대행
  "뷰티 HR PM·잡매니저": ["hr", "pm", "잡매니저", "job manager", "노무", "근태", "파견", "도급", "관리소장"],
