...
//
// 2분인 이유: 일이 관리자 화면 안에서 도는 되풀이라(원문을 띄워 놓고 불러온 값을 대조)
// 자리를 오래 비울 일이 없다. 다만 원문 창을 스크롤하며 대조하는 동안에는
// 우리 화면에 아무 신호가 오지 않는다(다른 창의 조작은 들을 수 없다).
// 1분으로 조이면 그 대조 시간이 통째로 빠진다.
export const IDLE_GAP_MIN = ALBA_IDLE_GAP_MIN;
