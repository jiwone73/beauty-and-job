...
const 항목머리 = /^[\s\u3000]*(?:\(?\d{1,2}\s*[).]|[◇◆■□▶▷※]|[【\[])[\s\u3000]*/;

/** 라벨 이름만 남긴다: "1) 전화번호 :" → "전화번호" */
function 라벨(line: string): string {
  return line.replace(항목머리, "").replace(/[:：].*$/, "")
...
export function 양식읽기(text: string): Record<string, string> {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!val) {
      const buf: string[] = [];
      for (let j = i + 1; j < lines.length && !항목머리.test(lines[j]); j++) buf.push(lines[j]);
...
const 칸이름: Record<string, string[]> = {
  ...
  work_time: ["근무시간", "영업시간", "근무시간대"],
  career: ["경력", "자격요건", "지원자격"],
  salary: ["급여", "급여조건", "월급", "시급"],
  ...
  extra_notes: ["조건", "근무조건", "지원방법", "하고픈말", "기타"],
};
