...
const 칸이름: Record<string, string[]> = {
  // 「매장이름 / 근무처」처럼 적는 양식이 있다. 목록에 없어 통째로 버려졌다 —
  ...
  job_category_raw: ["모집분야", "모집부문", "채용분야", "구인분야", "직무", "담당업무", "희망직원", "모집인원", "구인"],
...
const 고용형태모양 = /정규직|계약직|아르바이트|알바|파트타임|파트\s*타임|프리랜서|스페어|스페아|일용직|3\.3\s*%/;
const 고용형태맞춤: Record<string, string> = {
  "파트타임": "알바", "파트 타임": "알바", "아르바이트": "알바", "스페아": "스페어", "3.3%": "프리랜서", "3.3 %": "프리랜서",
};
...
export function parsePasted(text: string, 제목 = ""): PastedResult | null {
  ...
  //
  // 제목을 먼저 본다. 예전에는 본문 앞 네 줄만 봤는데, 머리말이나 인사말이 앞에
  // 붙은 글은 그 네 줄 안에 가게 이름이 없어 빈손으로 끝났다 — 정작 제목에는
  // 또렷이 적혀 있었다. 본문도 앞 여덟 줄까지 넓힌다.
  if (!out.company_name) {
    const v = 모양읽기([제목, ...줄들.slice(0, 8)].filter(Boolean), 매장이름모양);
