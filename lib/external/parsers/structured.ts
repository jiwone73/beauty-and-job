...
function parseHairinjob(html: string): StructuredResult | null {
  ...
  if (address) {
    const 칸 = address.trim().split(/\s+/);
    if (칸.length > 1) {
      const 푼것 = 묶음풀기(칸[0], 칸[1] || "");
      if (푼것 !== 칸[0]) address = [SIDO[푼것] || 푼것, ...칸.slice(1)].join(" ");
    }
  }
  if (address && region) {
  ...
  const stripLines = (s: string) =>
    // 주석은 태그 제거보다 먼저 걷어낸다. 안 그러면 '-->' 가 본문 꼬리로 남는다.
    s.replace(/<!--[\s\S]*?-->/g, " ")
      .replace(/<\s*br\s*\/?>/gi, "\n")
  ...
  let descText = "";
  {
    ...
    if (mIdx >= 0) {
      ...
      let s = -1;
      for (let p = t.indexOf("상세내용"); p >= 0; p = t.indexOf("상세내용", p + 1)) {
        if (!/^상세\s*내용을?\s*더/.test(t.slice(p, p + 12))) { s = p; break; }
      }
      if (s >= 0) t = t.slice(s + 4);
      // 사이트가 붙인 영역(로그인 안내·근무위치 지도·저작권/신고·스크립트)에서 끊는다.
      //
      // 여기 넣는 마커는 '사이트가 쓴 것이 확실한 말'이어야 한다.
      //   예전에는 "온라인 상시모집·전화 연결 안될시·문자로 지원 가능" 같은 흔한 안내 문구까지
      //   마커로 삼았는데, 그건 매장이 직접 쓴 본문에도 자주 나온다.
      //   가장 앞선 마커에서 자르므로, 그런 말이 글 앞부분에 있으면 본문이 통째로 날아갔다.
      //   (등록 이슈로 "상세요강 누락"이 여러 건 올라온 원인이 이것이다.)
      // 애매한 문구는 자르지 않고, 아래 SITE_BOILERPLATE 에서 그 문장만 지운다.
      t = t.split(/※\s*위\s*템플릿|※\s*템플릿으로\s*제작|상세\s*내용을?\s*더?\s*확인하시려면|로그인\s*하러\s*가기|로그인\s*후\s*(?:이용|확인)|근무\s*위치|접수\s*기간\s*및\s*방법|위\s*임플릿|저작권법|무단[^\n]*이용|신고하기|\$\(\s*document|naver\.maps|function\s*\(/)[0];
...
function mapRole(text: string): { job_type: string; item: string } | null {
  ...
  const R: [RegExp, string, string][] = [
    [/발관리|발 관리|풋케어|footcare|내성발톱|굳은살|티눈/, "STORE", "문제성 네일 손발톱 관리사"],
 ...
 *  그대로 두면 「전남광주 북구」 같은 없는 지역이 되어 검색에도 안 걸린다.
 *  묶음에서 실제 시·도를 골라내되, 어느 쪽인지는 뒤따르는 시·군·구로 가른다. */
const 묶음시도: { 표기: RegExp; 후보: string[] }[] = [
  { 표기: /^전남[·\/ ]?광주$|^광주[·\/ ]?전남$/, 후보: ["광주", "전남"] },
  { 표기: /^전북[·\/ ]?전남$/, 후보: ["전북", "전남"] },
  { 표기: /^대전[·\/ ]?충남$|^충남[·\/ ]?대전$/, 후보: ["대전", "충남"] },
  { 표기: /^대구[·\/ ]?경북$|^경북[·\/ ]?대구$/, 후보: ["대구", "경북"] },
  { 표기: /^부산[·\/ ]?경남$|^경남[·\/ ]?부산$/, 후보: ["부산", "경남"] },
  { 표기: /^울산[·\/ ]?경남$/, 후보: ["울산", "경남"] },
  { 표기: /^경기[·\/ ]?인천$|^인천[·\/ ]?경기$/, 후보: ["경기", "인천"] },
  { 표기: /^강원[·\/ ]?충북$/, 후보: ["강원", "충북"] },
];
/** 광역시의 구 이름 — 묶음 표기에서 어느 쪽인지 가르는 열쇠다. */
const 광역구: Record<string, string[]> = {
  광주: ["동구", "서구", "남구", "북구", "광산구"],
  대전: ["동구", "중구", "서구", "유성구", "대덕구"],
...
function 묶음풀기(첫칸: string, 다음칸: string): string {
  for (const { 표기, 후보 } of 묶음시도) {
    if (!표기.test(첫칸)) continue;
    // 뒤에 붙은 말이 그 광역시의 구 이름이면 광역시, 아니면 도(道)로 본다.
    const 광역 = 후보.find((c) => 광역구[c]?.includes(다음칸));
    return 광역 || 후보[1] || 후보[0];
  }
  return 첫칸;
}
...
function normRegionLoose(raw: string): string {
  ...
  const 첫 = 묶음풀기(parts[0], parts[1] || "");
  const sido = SIDO[첫] || SIDO[parts[0]] || parts[0];
...
function parseBeautyjob(html: string): StructuredResult | null {
  ...
  let industry = "";
  {
    ...
    else if (/발관리|발 관리|풋케어|내성발톱|굳은살|티눈/.test(ck)) { mappedCats = ["문제성 네일 손발톱 관리사"]; industry = "피부·에스테틱"; }
...
// ───────────── 셀렉미(selectme.co.kr) ─────────────
// 상세: /recruit/<id> (Next.js App Router, RSC). 데이터는 self.__next_f 스트림 JSON에 서버렌더.
//   공고 객체: {"isInvisibleClosedRecruit":..,"status":"ing",.,"shopName","title","contents","contentsImages",
//              "shopAddress","businessTypes":[{"name":직종,"conditions":[{type,headCount,salaryAmount{amount}}]},..],
//              "startWorkTime","endWorkTime","genderPriorityType","workType",.., "id":<recruitId>,"ceoId":..}
//   상세페이지엔 관련공고가 섞이므로 URL의 recruitId로 본 공고 객체를 특정한다.
function mapSelectmeCat(name: string): string {
...
function parseSelectme(html: string, url?: string): StructuredResult | null {
  ...
  const shopAddrDetail = g(/"shopAddressDetail":"([^"]*)"/).trim();
  const region = regionFromAddrText(shopAddress) || normRegionLoose(shopAddress);
  ...
  const isDayConsult = /"isWorkDayConsult":true/.test(chunk);
  const st = g(/"startWorkTime":"(\d{1,2}:\d{2})"/);
  const et = g(/"endWorkTime":"(\d{1,2}:\d{2})"/);
  const work_time = isTimeConsult ? "협의" : (st && et ? parseWorkTime(`${st}~${et}`) : "");
  const work_days = isDayConsult ? "협의" : "";

  // 성별우대
  const gp = g(/"genderPriorityType":"([^"]*)"/);
  const gender_preference = /female|여/i.test(gp) ? "여성" : /male|남/i.test(gp) ? "남성" : /unrelated|무관/i.test(gp) ? "무관" : "";

  // 직종(businessTypes): 이름 → 직군, 첫 조건에서 급여·경력·인원
  const btBlock = (chunk.match(/"businessTypes":\[([\s\S]*?)\](?=,"[a-zA-Z])/) || [])[1] || "";
  const names = [...btBlock.matchAll(/"name":"([^"]*)"/g)].map((m) => m[1]);
  const mappedCats = [...new Set(names.map(mapSelectmeCat).filter(Boolean))];
  const firstCond = (btBlock.match(/"conditions":\[\{([\s\S]*?)\}\}/) || btBlock.match(/"conditions":\[\{([\s\S]*?)\}/) || [])[1] || "";
  let career = "", salary = "", salary_type = "", salary_amount = 0, salary_negotiable = false, headcount = 0;
  if (firstCond) {
    const ctype = (firstCond.match(/"type":"([^"]*)"/) || [])[1] || "";
    career = /newcomer/.test(ctype) ? "신입" : /career/.test(ctype) ? "무관" : "";
  ...
  let industry = "";
  {
    const ck = `${names.join(" ")} ${title}`;
    if (/네일/.test(ck)) industry = "네일샵";
    else if (/속눈썹|왁싱|반영구|래쉬/.test(ck)) industry = "속눈썹·왁싱·반영구";
    else if (/피부|에스테틱|관리사/.test(ck)) industry = "피부·에스테틱";
    else if (/메이크업|분장/.test(ck)) industry = "메이크업";
