 ...
 */
export const 오픈일 = "2026-10-12";
/** 화면에 적는 오픈일. 「2026년 10월 12일」·「10월 12일」 두 벌이면 날짜를
 *  옮길 때 한쪽이 반드시 남는다 — 여기서 만들어 쓴다. */
export const 오픈일글 = (해포함 = false) => {
  const [y, m, d] = 오픈일.split("-").map(Number);
  return `${해포함 ? `${y}년 ` : ""}${m}월 ${d}일`;
};
...
export const LAUNCH_TASKS: LaunchTask[] = [
  ...
  { id: "sms-number", group: "바깥 기관", owner: "관리자", title: "문자 발신번호(02) 개통·등록",
    start: "2026-09-14", due: "2026-09-18", note: "하이어스 명의 기업 인터넷전화 02 개통 → 이용증명원 → 네이버클라우드 서류 인증" },
  ...
  { id: "otp-fallback", group: "클로드", owner: "클로드", title: "휴대폰 인증 임시 폴백 걷기 · 발신번호 교체",
    start: "2026-09-18", due: "2026-09-22", after: "sms-number",
    note: "발송 실패 시 인증번호가 응답에 실려 나가는 임시 코드 — 오픈 전에 반드시 걷는다" },
